...
ELEVENLABS_VOICE_ID = os.getenv('ELEVENLABS_VOICE_ID', 'onwK4e9ZLuTAKqWW03F9')  # '21m00Tcm4TlvDq8ikWAM'
VOSK_MODEL_PATH = os.getenv('VOSK_MODEL_PATH', './vosk-model-en-us-0.22-graph')
ELEVENLABS_API_KEY = os.getenv("ELEVENLABS_API_KEY")
twilio_accnt = os.getenv("TWILIO_ACCOUNT_SID")
twilio_auth = os.getenv("TWILIO_AUTH_TOKEN")

# openai.api_key = OPENAI_API_KEY
twilio_client = Client(twilio_accnt, twilio_auth)
model = vosk.Model(VOSK_MODEL_PATH)
contacts_file = os.getenv("CONTACTS_FILE_PATH")
app = FastAPI()

@app.post('/call')
# Contacts_file path is put in .env file
async def handle_outgoing_calls():
    
    try:
        # Make an outgoing call
        CallNumber.make_call(contacts_file)
        ...
        # Use generate_audio() to dynamically include audio content in the response
        intro_audio = TextToSpeech.generate_intro()
        response.play(intro_audio)

        return str(response)

    except Exception as e:
        # Handle any exceptions that might occur during the call initiation or TwiML response generation
        print(f"Error: {str(e)}")
        return "Error during call initiation"

# Replace "http://your-websocket-stream-url/{call_sid}" with the actual URL of your WebSocket stream.
...
async def wait_for_user_input(websocket):
    ...
    try:
        ...
        print(f"Error in wait_for_user_input: {str(e)}")
